...
def get_vehicles_dic(db):
    ...
    for row in cursor.execute(sql):
        ...
        name  =  row[7]
        groupname =  row[11]
        createts  =  row[3][:4] if row[3] is int else None
        ...
        org_id = row[8] if type(row[8]) is not 'NoneType' else None
        org_name = row[10] if type(row[10]) is not 'NoneType' else None
        org_inn = row[9] if type(row[9]) is not 'NoneType' else None
         # Response structure
        vehicles.append({
            'id': id,
            'idDev' : idDev,
...
def get_t_by_device(idDev,gRPC_URL):
    ...
    try:
        ...
        print(str(exp))
    print(len(grpc_message))

    id = vehicles_attrib[0][0]
    idDev = vehicles_attrib[0][1]
    typets = vehicles_attrib[0][2] if type(vehicles_attrib[0][2]) is not 'NoneType' else None
    ...
    lat = grpc_message[0][4] if len(grpc_message) > 0 else None
    lon = grpc_message[0][3] if len(grpc_message) > 0 else None
    speed = grpc_message[0][6] if len(grpc_message) > 0 else None
    angle = grpc_message[0][5] if len(grpc_message) > 0 else None
    time = grpc_message[0][1] if len(grpc_message) > 0 else None
    date = datetime.datetime.fromtimestamp(grpc_message[0][1]).strftime('%Y-%m-%d %H:%M:%S') if len(grpc_message) > 0 else None
